...
driver.set_page_load_timeout(60)  # 设置页面加载超时时间为60秒

# 打开网页
driver.get('https://www.ymt.com/supplylist/main')
time.sleep(2)
list1 = []
list2 = []
list3 = []
...
# 循环获取省份项
try:
    province_items = driver.find_elements(By.CLASS_NAME, "supplylist_filter_item_level0")
    ...
    for item in province_items:
        ...
        product_links = driver.find_elements(By.CLASS_NAME, "supplylist_filter_item_level1")
        countproduct_link = 0
        for pro in product_links:
            product_links = driver.find_elements(By.CLASS_NAME, "supplylist_filter_item_level1")
            text = product_links[countproduct_link].find_element(By.TAG_NAME, "a").text
            count2 += 1
            list2.append({
               "id": count2 + 100000,
               "parent_id": count1,
            ...
            countproduct_link += 1
            breed_items = driver.find_elements(By.CLASS_NAME, "supplylist_filter_item")
            countbreed_items = 0
            for breed in breed_items:
                count3 += 1
                text = breed.find_element(By.TAG_NAME, "a").text
                list3.append({
...
# 过滤掉 label 为 "不限" 的元素
list3 = [item for item in list3 if item["label"] != "不限"]

# 过滤掉 label 最后一个字符是“省”、“市”或“区”的数据
list3 = [item for item in list3 if isinstance(item["label"], str) and item["label"] and item["label"][-1] not in ["省", "市", "区"]]
