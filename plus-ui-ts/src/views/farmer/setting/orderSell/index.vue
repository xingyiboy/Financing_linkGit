      ...
      <el-table v-loading="loading" :data="goodsOrderList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-row :gutter="20">
              <el-col :span="24">
                <el-button @click="router.push(`/detail/${scope.row.goodsId}`);" style="width: 100%;">跳转</el-button>
              </el-col>
              <el-col :span="24">
                <el-button style="width: 100%;" v-if="scope.row.status == 1"
                  @click="handleUpdate(scope.row)">已发货</el-button>
              </el-col>
              <el-col :span="24">
                <el-button style="width: 100%;" @click="handleDelete(scope.row)"
                  v-hasPermi="['financinglink:goodsOrder:remove']">删除</el-button>
              </el-col>
            </el-row>
          </template>
...
const handleUpdate = async (row) => {
  await proxy?.$modal.confirm('是否确认发货商品订单编号为"' + row.id + '"的数据项？').finally(() => loading.value = false);
  row.status = 2
