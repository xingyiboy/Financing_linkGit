  ...
  <div class="p-2">
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['financinglink:appointment:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['financinglink:appointment:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['financinglink:appointment:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['financinglink:appointment:export']">导出</el-button>
          </el-col>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['financinglink:appointment:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['financinglink:appointment:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="appointmentFormRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="种植作物" prop="crop">
          <el-input v-model="form.crop" placeholder="请输入种植作物" />
        </el-form-item>
        <el-form-item label="种植作物详细信息" prop="cropDetail">
          <el-input v-model="form.cropDetail" placeholder="请输入种植作物详细信息" />
        </el-form-item>
        <el-form-item label="作物条件" prop="cropCondition">
          <el-input v-model="form.cropCondition" placeholder="请输入作物条件" />
        </el-form-item>
        <el-form-item label="土壤条件" prop="edaphicCondition">
          <el-input v-model="form.edaphicCondition" placeholder="请输入土壤条件" />
        </el-form-item>
...
const submitForm = () => {
  appointmentFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        await updateAppointment(form.value).finally(() =>  buttonLoading.value = false);
      } else {
        await addAppointment(form.value).finally(() =>  buttonLoading.value = false);
      }
